...
namespace {
   ...
   public:
      ...
      llvm::FunctionCallee getGcReadWeakFn() {
         ...
         llvm::Type *args[] = { llvm::PointerType::get(CGM.getLLVMContext(), 0) };
         llvm::FunctionType *FTy =
   ...
   public:
      ...
      llvm::FunctionCallee getSetJmpFn() {
         ...
         llvm::Type *params[] = { llvm::PointerType::get(CGM.getLLVMContext(), 0) };
         return
...
llvm::StructType *CGObjCCommonMulleRuntime::CreateNSConstantStringType( void)
{
   ...
   D = RecordDecl::Create(Context, TagTypeKind::Struct, Context.getTranslationUnitDecl(), Loc, Loc,
...
ConstantAddress CGObjCCommonMulleRuntime::GenerateConstantString( const StringLiteral *SL)
{
   ...
   //
   if( ! this->no_tagged_pointers && SL->getKind() == StringLiteralKind::Ordinary)
   {
                                        ...
                                        bool hasRvalType)
{
   ...
   UD = RecordDecl::Create( *context, TagTypeKind::Union, context->getTranslationUnitDecl(), Loc, Loc,
                              &context->Idents.get("_u_args"));
   UD->setImplicit();
   UD->addAttr(TypeVisibilityAttr::CreateImplicit(
        *context, TypeVisibilityAttr::Default));

   UD->setTagKind( TagTypeKind::Union);
   UD->startDefinition();
                                                          ...
                                                          ArrayRef< QualType> Types)
{
   ...
   RecordDecl  *RD = RecordDecl::Create( *Context, TagTypeKind::Struct, OnTheFlyMethodDecl, SourceLocation(), SourceLocation(), RecordID);
                                                                   ...
                                                                   llvm::ArrayRef<const Expr*> &Exprs)
{
   ...
   // (DeclContext *) CGF.CurFuncDecl is a hack
   RecordDecl  *RD2 = RecordDecl::Create( *Context, TagTypeKind::Struct, OnTheFlyMethodDecl, SourceLocation(), SourceLocation(), RecordID);
                                            ...
                                            RecordDecl *UD)
{
   ...
   LValue   Alloca  = CGF.MakeNaturalAlignAddrLValue( allocaed.getBasePointer(), Ty);
...
//   };
      ...
      Values[ i++] = llvm::ConstantExpr::getBitCast(
                        _HashConstantForString( ID->getObjCRuntimeNameAsString().str()),
                        ObjCTypes.ClassIDTy);
   ...
   else
   {
      ...
      unsigned Size =
      CGM.getContext().getASTObjCInterfaceLayout(ID->getClassInterface()).getSize().getQuantity();

      SmallVector<llvm::Constant *, 16> InstanceVariables;
                                          ...
                                          ArrayRef<llvm::Constant*> Classes)
{
   ...
   llvm::GlobalVariable *GV = CreateMetadataVar( Name, Init, Section, CGM.getPointerAlign());
   return llvm::ConstantExpr::getBitCast(GV, llvm::PointerType::get(CGM.getLLVMContext(), 0));
}
                                          ...
                                          ArrayRef<llvm::Constant*> Categories)
{
   ...
   llvm::ArrayType *AT = llvm::ArrayType::get( llvm::PointerType::get(CGM.getLLVMContext(), 0),
                                              Categories.size());
...
llvm::Function *CGObjCMulleRuntime::ModuleInitFunction() {
  ...
  llvm::FunctionType *FT =
    llvm::FunctionType::get(Builder.getVoidTy(),
                            llvm::PointerType::get(CGM.getLLVMContext(), 0), true);
...
namespace {
  struct PerformFragileFinally final : EHScopeStack::Cleanup {
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      llvm::Value *TryExitArgs[] = { ExceptionData.getBasePointer(), UniverseID };
...
static void addIfPresent(llvm::DenseSet<llvm::Value*> &S, Address V) {
  if (V.isValid()) S.insert(V.getBasePointer());
}
...
void CGObjCMulleRuntime::EmitTryOrSynchronizedStmt(CodeGen::CodeGenFunction &CGF,
   ...
   llvm::Value *TryEnterArgs[] = { ExceptionData.getBasePointer(), UniverseID };
  ...
  SetJmpResult->setCanReturnTwice();
   ...
   if (!isTry || !cast<ObjCAtTryStmt>(S).getNumCatchStmts()) {
      ...
      llvm::Value *CaughtArgs[] = { ExceptionData.getBasePointer(), UniverseID };
...
llvm::Value * CGObjCMulleRuntime::EmitObjCWeakRead(CodeGen::CodeGenFunction &CGF,
  ...
  llvm::Value *AddrWeakObjVal = CGF.Builder.CreateBitCast(
      AddrWeakObj.getBasePointer(), ObjCTypes.PtrObjectPtrTy);
...
void CGObjCMulleRuntime::EmitObjCWeakAssign(CodeGen::CodeGenFunction &CGF,
  ...
  llvm::Value *dstVal =
      CGF.Builder.CreateBitCast(dst.getBasePointer(), ObjCTypes.PtrObjectPtrTy);
...
void CGObjCMulleRuntime::EmitGCMemmoveCollectable(CodeGen::CodeGenFunction &CGF,
  ...
  llvm::Value *args[] = { DestPtr.getBasePointer(), SrcPtr.getBasePointer(), size };
...
llvm::Constant *CGObjCCommonMulleRuntime::GetIvarType(const ObjCIvarDecl *Ivar)
{
   ...
   // on assignment
   //
   // @mulle-objc@ ivar type: don't expand pointed-to structs/unions >
   //
   // A pointer to a struct/union is always pointer-sized regardless of the
   // pointee's definition. We only expand embedded (by-value) structs/unions
   // since those affect sizeof. Expanding pointees would make this string
   // depend on struct visibility at codegen time, which is at best
   // inconsistent and at worst wrong (see also getIvarHashString).
   //
...
: VMContext(cgm.getLLVMContext()), CGM(cgm), ExternalProtocolPtrTy(nullptr)
{
   ...
   ClassIDPtrTy      = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   CategoryIDTyPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   SelectorIDTyPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   SuperIDTyPtrTy    = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   PropertyIDTyPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   IvarIDPtrTy       = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   ProtocolIDPtrTy   = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   ...
   UniversePtrTy    = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   ...
   // struct _prop_list_t *
   PropertyListPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   ...
   CachePtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
...
: ObjCCommonTypesHelper(cgm) {
   ...
   // struct _objc_method_description_list *
   MethodDescriptionListPtrTy =  llvm::PointerType::get(CGM.getLLVMContext(), 0);
   ...
   SuperListPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   ...
   ProtocolListTy->setBody({llvm::PointerType::get(CGM.getLLVMContext(), 0),
                           LongTy,
                           llvm::ArrayType::get(ProtocolTy, 0)});

   // struct _objc_protocol_list *
   ProtocolListPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);

   ProtocolPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
   ...
   IvarListPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);

   // struct _objc_method_list *
   MethodListTy =
   llvm::StructType::create(VMContext, "struct._mulle_objc_methodlist");
   MethodListPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);

   // struct _objc_class_extension *
   ClassExtensionTy =
   llvm::StructType::create("struct._objc_classextension",
                            IntTy, Int8PtrTy, PropertyListPtrTy);
   ClassExtensionPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
...
//   };
   ...
   ClassTy->setBody(
                    ...
                    ClassIDTy,         // classivarhash
                    ClassIDTy,         // superclassid
                    Int8PtrTy,         // superclassname
...
//   };
   ...
   SymtabPtrTy = llvm::PointerType::get(CGM.getLLVMContext(), 0);
