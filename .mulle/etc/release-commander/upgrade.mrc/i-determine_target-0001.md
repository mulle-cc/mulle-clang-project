Determine the target llvm major version to upgrade to.
...
2. Find the current major version from `migrate-to-next-release`:
   ```bash
   grep '^NEW_LLVM_TAG=' clang/bin/migrate-to-next-release
   # e.g. llvmorg-21.1.8 → current major = 21
   ```
...
5. Pick the highest patch release of `target_major`:
   ```bash
   TARGET_MAJOR=22   # replace with current+1
